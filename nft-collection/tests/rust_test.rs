...
use nft_collection::structs::{CollectionInfo, MintPrice, TimePeriod, TierInfoEntry, COLLECTION_HASH_LEN};
use nft_collection::views::ViewsModule;
use nft_collection::storage::StorageModule;
use nft_collection::private_functions::PrivateFunctionsModule;
use nft_collection::DutyNftMinter;
use nft_collection_interactor::*;

#[test]
fn init_test() {
    let _ = DutyNftMinterSetup::new(nft_collection::contract_obj);
}

#[test]
fn create_collections_test() {
    let mut nm_setup = DutyNftMinterSetup::new(nft_collection::contract_obj);
    nm_setup.create_default_collections();

    // try create collection, same collection
    nm_setup
        .call_create_new_collection(
    ...
    // try create collection, same collection ID
    nm_setup
        .call_create_new_collection(
    ...
    // try create collection, unsupported media type
    nm_setup
        .call_create_new_collection(
            THIRD_COLLECTION_HASH,
            THIRD_COLLECTION_ID,
    ...
    nm_setup
        ...
        .execute_query(&nm_setup.nm_wrapper, |sc| {
            let result = sc.get_collection_info_view(managed_buffer!(FIRST_COLLECTION_ID));

            let expected_collection_id = managed_buffer!(FIRST_COLLECTION_ID);
            assert_eq!(result.collection_id, expected_collection_id);

            let expected_token_id = managed_token_id!(FIRST_TOKEN_ID);
            assert_eq!(result.nft_token_id, expected_token_id.unwrap_esdt());

            let expected_collection_info = CollectionInfo::<DebugApi> {
            ...
            assert_eq!(result.collection_info, expected_collection_info);
    ...
    nm_setup
        ...
        .execute_query(&nm_setup.nm_wrapper, |sc| {
            let mapper = sc.available_ids(
                &managed_buffer!(FIRST_COLLECTION_ID),
            );
    ...
    nm_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                sc.add_to_whitelist(managed_buffer!(FIRST_COLLECTION_ID), args);
            },
        )
        .assert_ok();
...
fn giveaway_test() {
    ...
    nm_setup
        ...
        .execute_query(&nm_setup.nm_wrapper, |sc| {
            let mapper = sc.available_ids(
                &managed_buffer!(SECOND_COLLECTION_ID),
            );
